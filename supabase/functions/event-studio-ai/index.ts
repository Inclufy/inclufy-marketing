...
async function handleOcrCard(body: Record<string, unknown>) {
  ...
  const systemPrompt = `You are an OCR specialist that extracts contact information from business cards.
Analyze the image and extract all visible contact details.
Always respond with valid JSON: {"first_name": string, "last_name": string, "email": string, "phone": string, "company": string, "title": string}
If a field is not visible or unclear, use an empty string "".
For names: split into first_name and last_name. If only one name is visible, put it in first_name.
Extract phone numbers in international format if possible.`;

  const userContent: object[] = [
    { type: 'text', text: 'Extract all contact information from this business card image. Return JSON with: first_name, last_name, email, phone, company, title.' },
  ...
  return parseJSON(result, { first_name: '', last_name: '', email: '', phone: '', company: '', title: '' });
}
